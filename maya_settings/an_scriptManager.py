...
def scriptManager(path ='', scriptFolder=''):
    ...
    if not path:
        if __name__ == '__main__':
            path = os.path.abspath(r'D:/andreikin/scripts')
        else:
...
def an_sourceProcedures (path):
    ...
    procPath = get_folders_path(path, PROCEDURES)
    print 'Source procedures from: ', procPath
    if procPath:
...
#zaplatka:
wPath ='//dataServer/Project/backup_s3d/lib/setup/maya/maya_scripts_rfm3/rigging_tools/procedures/'
if wPath  in list(sys.path):
	sys.path.remove(wPath)

	#'D:\andreikin\skripts\procedures'
